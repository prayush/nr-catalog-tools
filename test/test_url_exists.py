...
from nrcats.utils import url_exists
...
def test_url_exists_returns_false_after_all_retries_fail():
    ...
    with patch(
        "nrcats.utils.requests.head", side_effect=OSError("network error")
    ), patch("nrcats.utils.time.sleep"):
...
def test_url_exists_returns_true_after_transient_failure():
    ...
    with patch("nrcats.utils.requests.head", side_effect=side_effects), patch(
        "nrcats.utils.time.sleep"
    ):
...
def test_url_exists_returns_false_on_non_200_status():
    ...
    bad = MagicMock()
    bad.status_code = 404
    with patch("nrcats.utils.requests.head", return_value=bad):
...
def test_url_exists_retry_count_is_respected():
    ...
    with patch(
        "nrcats.utils.requests.head", side_effect=OSError("fail")
    ) as mock_head, patch("nrcats.utils.time.sleep"):
