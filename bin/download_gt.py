...
files using two separate catalog toolkits: `mayawaves` and `nrcats`.

Workflow:
----------
1. The script initializes two catalog handlers:
   - `mayawavescatalog` using mayawaves' `Catalog` class.
   - `mayacatalog` using nrcats' `MayaCatalog`.
...
3. For any simulations whose data failed to download in the previous step, the script
   attempts to fetch the remaining missing waveform files using the `nrcats` library.

Intended Use:
-------------
- This script is typically run in a user environment where the `mayawaves` and
  `nrcats` Python libraries are available and properly configured.
